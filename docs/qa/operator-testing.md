...
| 1.4 | Open `lossy-test.md`; encoding menu → **Convert to… → Windows-1254**; Save | **Lossy dialog** appears (3 options): Save as UTF-8 / Save anyway / Cancel 🆕 |✅ |
| 1.5 | In that dialog, pick **Save as UTF-8** | Saves losslessly; encoding now shows **UTF-8**; no data lost. ⚠ This **resets the save target to UTF-8** — see note below |✅ |
| 1.6 | **Convert to Windows-1254 again** (1.5 reset it to UTF-8), Save, pick **Save anyway** | Saves in Windows-1254; unrepresentable chars replaced (data loss, as warned). Afterwards restore the fixture: `git checkout docs/qa/fixtures/lossy-test.md` |✅ |
| 1.7 | Reopen a file, use status-bar **Reopen with encoding** on a legacy file | Re-decodes; not marked dirty |✅ |

> **Why the lossy dialog seems to "not reopen":** it is **state-driven, not
> one-shot** — it fires whenever the *current* save-target encoding can't
> represent the buffer. Picking **Save as UTF-8** switches the target to UTF-8
> (lossless), so a plain Ctrl+S afterwards won't re-open it; **Convert back to
> Windows-1254 first**. Also use **Convert to…**, *not* **Reopen with…** —
> Reopen re-decodes the bytes as Windows-1254 (mojibake) and won't be lossy on
> save. This is expected behavior, not a bug (`save_file` is stateless).
